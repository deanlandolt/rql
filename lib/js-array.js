...
({define:typeof define!="undefined"?define:function(deps, factory){module.exports = factory(exports, require("./parser"), require("./query"), require("assert"));}}).
define(["exports", "./parser", "./query", "assert"], function(exports, parser, QUERY, ASSERT){
...
exports.operators = {
	...
	one: function(){
		ASSERT.ok(this.length <= 1, "More than one object found");
		return this[0];
	},
	assert: function(func) {
		if (this == null) return this;
		var target = this;
		// if scalar temporarily wrap to apply
		if (typeof target.forEach != "function") {
			// unbox primitives
			if (target instanceof Number) target = +target;
			else if (target instanceof String) target = "" + target;
			target = [target];
		}
		ASSERT.deepEqual(target, func.call(target), "context: " + JSON.stringify(this));
		return this;
	}
};
...
function filter(condition, not){
	...
	var filter = function(property, second){
		...
		for(var i = 0, length = this.length; i < length; i++){
			var item = this[i];
			if (item != null) { // skip null and undefined
...
function query(query, options, target){
	...
	var js = "";
	var getFirst;
	if (typeof target != "undefined" && (!target || typeof target.forEach != "function")) {
		target = [target];
		getFirst = true;
	}
	...
	if (!target) return evaluator;
	var result = evaluator(target);
	if (getFirst && result && typeof result.forEach == "function") return result[0];
	return result;
}
